  ...
  <view class="address">
    <view class="address_wu" v-if="addressList.length == 0">
      <view class="address_wu_img center">
        <image src="https://static.gute.fun/static/luckyOwn/publice/zanwushouhuo@2x.png" mode=""></image>
        <text>暂无收款账号</text>
      </view>
      <button class="address_wu_btn" hover-class="hover-view" @click="address(0)">添加收款账号</button>
    </view>
      ...
      <view class="address_have_ul">
        <view class="address_have_li flex" v-for="(item,index) in addressList" :key="index" @click="selectAddress(item)">
          ...
          <view class="address_have_li_edit" @click.stop="editAddress(item)">
            <image src="https://static.gute.fun/static/luckyOwn/me/bianji@2x.png" mode=""></image>
          </view>
        </view>
      </view>
      <button class="address_have_btn address_wu_btn" @click="address(0)">新增收款账号</button>
    </view>
  </view>
</template>
...
export default {
  ...
  methods:{
    ...
    editAddress (item) {
      ...
      uni.navigateTo({url:'/subPages/addCollectMoney/addCollectMoney?type=1'})
    },
    //选择地址
    selectAddress (item) {
    ...
    address (type) {
      uni.navigateTo({url:'/subPages/addCollectMoney/addCollectMoney?type=' + type})
    },
    //获取收款地址
    getAddressList () {
